...
Bartik_dataW <- merge(filter(region_data,REGION==1), data_global) %>%
  ...
  summarise(Bartik = sum(prop*Agg_pe_ind), Bartik5 = sum(prop*Agg_pe5_ind,na.rm=TRUE))%>%
  mutate(REGION =1)
...
panel_data <- full_join(master_data, Bartik_data) %>%
  merge(unEm) 

#panel_data <- panel_data[c("REGION", "survey_date", "pe", "pe5", "RegInf", "Bartik", "UNRATE")]

panel_data_First <- full_join(master_data_First, Bartik_data_First) %>%
  merge(unEm) 


#panel_data_First <- panel_data_First[c("REGION", "survey_date", "pe", "RegInf", "Bartik", "UNRATE")]

panel_data_Small <- full_join(master_data_Small, Bartik_data_Small) %>%
  merge(unEm) 
...
write_csv(region_data,"data/dataBuild/regiondatawith5.csv")
write_csv(panel_data,"data/dataBuild/paneldatawith5.csv")
write_csv(panel_data_First,"data/dataBuild/paneldataFirstwith5.csv")
write_csv(panel_data_Small,"data/dataBuild/paneldataSmallwith5.csv")
write_csv(master_data,"data/dataBuild/masterdatawith5.csv")
write_csv(master_data_First,"data/dataBuild/masterdataFirstwith5.csv")
write_csv(master_data_Small,"data/dataBuild/masterdataSmallwith5.csv")
write_csv(data_local,"data/dataBuild/localdatawith5.csv")
write_csv(data_local_First,"data/dataBuild/localdataFirstwith5.csv")
write_csv(data_local_Small,"data/dataBuild/localdataSmallwith5.csv")
write_csv(data_global,"data/dataBuild/globaldatawith5.csv")
write_csv(data_global_First,"data/dataBuild/globaldataFirstwith5.csv")
write_csv(data_global_Small,"data/dataBuild/globaldataSmallwith5.csv")
