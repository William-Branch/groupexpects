# Generate data frames for estimation;
...
Bartik_lagW <- merge(filter(region_data,REGION==1), data_global) %>%
  mutate(lagprop = lag(prop,12)) %>%
  group_by(survey_date) %>%
  summarise(Bartik_lag = sum(lagprop*Agg_pe_ind)) %>%
  mutate(REGION=1)

Bartik_lagMW <- merge(filter(region_data,REGION==2), data_global) %>%
  mutate(lagprop = lag(prop,12)) %>%
  group_by(survey_date) %>%
  summarise(Bartik_lag = sum(lagprop*Agg_pe_ind)) %>%
  mutate(REGION=2)

Bartik_lagNE <- merge(filter(region_data,REGION==3), data_global) %>%
  mutate(lagprop = lag(prop,12)) %>%
  group_by(survey_date) %>%
  summarise(Bartik_lag = sum(lagprop*Agg_pe_ind)) %>%
  mutate(REGION=3)

Bartik_lagS <- merge(filter(region_data,REGION==4), data_global) %>%
  mutate(lagprop = lag(prop,12)) %>%
  group_by(survey_date) %>%
  summarise(Bartik_lag = sum(lagprop*Agg_pe_ind)) %>%
  mutate(REGION=4)

Bartik_data_lag <- rbind(Bartik_lagW, Bartik_lagMW, Bartik_lagNE, Bartik_lagS)
...
Bartik_First_lagW <- merge(filter(region_data,REGION==1), data_global_First) %>%
  mutate(lagprop = lag(prop,12)) %>%
  group_by(survey_date) %>%
  summarise(Bartik_lag = sum(lagprop*Agg_pe_ind)) %>%
  mutate(REGION=1)

Bartik_First_lagMW <- merge(filter(region_data,REGION==2), data_global_First) %>%
  mutate(lagprop = lag(prop,12)) %>%
  group_by(survey_date) %>%
  summarise(Bartik_lag = sum(lagprop*Agg_pe_ind)) %>%
  mutate(REGION=2)

Bartik_First_lagNE <- merge(filter(region_data,REGION==3), data_global_First) %>%
  mutate(lagprop = lag(prop,12)) %>%
  group_by(survey_date) %>%
  summarise(Bartik_lag = sum(lagprop*Agg_pe_ind)) %>%
  mutate(REGION=3)

Bartik_First_lagS <- merge(filter(region_data,REGION==4), data_global_First) %>%
  mutate(lagprop = lag(prop,12)) %>%
  group_by(survey_date) %>%
  summarise(Bartik_lag = sum(lagprop*Agg_pe_ind)) %>%
  mutate(REGION=4)

Bartik_data_First_lag <- rbind(Bartik_First_lagW, Bartik_First_lagMW, Bartik_First_lagNE, Bartik_First_lagS)
...
Bartik_Small_lagW <- merge(filter(region_data,REGION==1), data_global_Small) %>%
  mutate(lagprop = lag(prop,12)) %>%
  group_by(survey_date) %>%
  summarise(Bartik_lag = sum(lagprop*Agg_pe_ind)) %>%
  mutate(REGION=1)

Bartik_Small_lagMW <- merge(filter(region_data,REGION==2), data_global_Small) %>%
  mutate(lagprop = lag(prop,12)) %>%
  group_by(survey_date) %>%
  summarise(Bartik_lag = sum(lagprop*Agg_pe_ind)) %>%
  mutate(REGION=2)

Bartik_Small_lagNE <- merge(filter(region_data,REGION==3), data_global_Small) %>%
  mutate(lagprop = lag(prop,12)) %>%
  group_by(survey_date) %>%
  summarise(Bartik_lag = sum(lagprop*Agg_pe_ind)) %>%
  mutate(REGION=3)

Bartik_Small_lagS <- merge(filter(region_data,REGION==4), data_global_Small) %>%
  mutate(lagprop = lag(prop,12)) %>%
  group_by(survey_date) %>%
  summarise(Bartik_lag = sum(lagprop*Agg_pe_ind)) %>%
  mutate(REGION=4)

Bartik_data_Small_lag <- rbind(Bartik_Small_lagW, Bartik_Small_lagMW, Bartik_Small_lagNE, Bartik_Small_lagS)
...
panel_data_lag <- merge(master_data, Bartik_data_lag) %>%
  merge(unEm)

panel_data_Small_lag <- merge(master_data_Small, Bartik_data_Small_lag) %>%
  merge(unEm)

panel_data_First_lag <- merge(master_data_First, Bartik_data_First_lag) %>%
  merge(unEm)
...
panel_data_First <- merge(panel_data_First, controls) %>%
  filter(!is.na(pe))

panel_data_Small <- merge(panel_data_Small, controls) %>%
  filter(!is.na(pe))

panel_data_lag <- full_join(panel_data_lag, controls) %>%
  filter(!is.na(pe))

panel_data_First_lag <- merge(panel_data_First_lag, controls) %>%
  filter(!is.na(pe))

panel_data_Small_lag <- merge(panel_data_Small_lag, controls) %>%
  filter(!is.na(pe))

write_csv(panel_data,"data/dataBuild/paneldata.csv")
write_csv(panel_data_lag, "data/dataBuild/paneldataLag.csv")
write_csv(panel_data_First_lag, "data/dataBuild/paneldataFirstLag.csv")
write_csv(panel_data_Small_lag, "data/dataBuild/paneldataSmallLag.csv")
write_csv(panel_data_First,"data/dataBuild/paneldataFirst.csv")
write_csv(panel_data_Small,"data/dataBuild/paneldataSmall.csv")
write_csv(region_data,"data/dataBuild/regiondata.csv")
write_csv(master_data,"data/dataBuild/masterdata.csv")
write_csv(master_data_First,"data/dataBuild/masterdataFirst.csv")
write_csv(master_data_Small,"data/dataBuild/masterdataSmall.csv")
write_csv(data_local,"data/dataBuild/localdata.csv")
write_csv(data_local_First,"data/dataBuild/localdataFirst.csv")
write_csv(data_local_Small,"data/dataBuild/localdataSmall.csv")
write_csv(data_global,"data/dataBuild/globaldata.csv")
write_csv(data_global_First,"data/dataBuild/globaldataFirst.csv")
write_csv(data_global_Small,"data/dataBuild/globaldataSmall.csv")
write_csv(local_dataBig, "data/dataBuild/localdataBig.csv")
write_csv(local_dataBig_Small, "data/dataBuild/localdataBigSmall.csv")
