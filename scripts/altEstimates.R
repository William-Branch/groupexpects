# altEstimates.R
#Start fresh with libraries
# Get the list of currently loaded packages
# loaded_packages <- search()[!(search() %in% c(".GlobalEnv", "package:stats", "package:graphics",  "package:grDevices", "package:utils", "package:datasets", "package:methods", "Autoloads", "package:base"))]
# 
# # Detach packages
...
source('functions/functions_extensions.R')
source('functions/functions_data.R')

#Load data
panel_out <- read_csv("data/dataBuild/panelcpsout.csv") %>% mutate(year = format(survey_date, "%Y"), yearquarter = as.yearqtr(survey_date, format = "%Y-%m-%d")) %>%  panel(~REGION + survey_date)
panel_bench_out <- read_csv("data/dataBuild/panelbenchout.csv")%>% mutate(year = format(survey_date, "%Y"), yearquarter = as.yearqtr(survey_date, format = "%Y-%m-%d"))  %>% panel(~REGION + survey_date)
panelNoOut <- read_csv("data/dataBuild/panelcps.csv")%>% mutate(year = format(survey_date, "%Y"), yearquarter = as.yearqtr(survey_date, format = "%Y-%m-%d")) %>%  panel(~REGION + survey_date)
...
statecpi_raw <- read_csv("data/statecpi_beta.csv")
...
# Start with Components. Lots of downloading, difficulty making modular.
#Non durables:
#west
...
print(table_out_cats)

# Save the final table as a LaTeX file
kbl(table_out_cats, "latex", booktabs = T, caption = "2SLS by component inflation", col.names = c( "commodities","non-durables", "durables", "services", "services-house", "services-med."), linesep = "\\addlinespace", label = "components:2sls:stage2") %>%
  ...
  footnote(general = "Reports panel regression results where the outcome variable is component inflation. Commodities include all non-durable and durable goods.  Services-housing and services-med. remove housing and medical services, respectively." , threeparttable = T) %>%
  add_header_above(c(" " = 1, "Commodities" = 3, "Services" = 3), color = "BillRed") %>%
  save_kable("tables/2slscomponents.tex")

# The aggregate pass-through effects.




panelout <- panel_out
panelbenchout <- panel_bench_out

MedExp <- fredr(series_id = "MICH") %>%
  filter(!is.na(value)) %>%
  mutate(medianExp = value, survey_date = date) %>%
  select(c(survey_date, medianExp))



transDat <- function(x, na.rm = TRUE)(mean(x, na.rm = na.rm))
deMeanDat <- function(x,y, na.rm = TRUE)( x-y+ mean(x, na.rm = na.rm))


rAvg <- panelout %>%
  group_by(survey_date) %>%
  #mutate_if(is.numeric, transDat)%>%
  mutate_at(vars(-REGION, -yearquarter), transDat, FALSE)%>%
  rename_with( ~ paste0("R", .x))%>%
  rename(survey_date = Rsurvey_date, REGION = RREGION)
rAvgNo <- panelNoOut %>%
  group_by(survey_date) %>%
  #mutate_if(is.numeric, transDat)%>%
  mutate_at(vars(-REGION, -yearquarter), transDat, FALSE)%>%
...
rAvgBenchOut <- panelbenchout %>%
  ...
  rename(survey_date = Rsurvey_date, REGION = RREGION)
tAvg <- panelout %>%
  group_by(REGION, yearquarter) %>%
  mutate_if(is.numeric, transDat)%>%
  rename_with( ~ paste0("T", .x))%>%
  rename(survey_date = Tsurvey_date, REGION = TREGION, yearquarter = Tyearquarter)
tAvgNo <- panelNoOut %>%
  group_by(REGION, yearquarter) %>%
  mutate_if(is.numeric, transDat)%>%
  rename_with( ~ paste0("T", .x))%>%
...
panelwAvg <- panelout %>%
  ...
  merge(tAvg) %>%
  merge(MedExp)
panelwAvgNoOut <- panelNoOut %>%
  merge(rAvgNo) %>%
  merge(tAvgNo) %>%
  merge(MedExp) %>%
  mutate(Bartik_out = panelwAvg$Bartik_out)

panelwAvgBenchOut <- panelbenchout %>%
  merge(rAvgBenchOut) %>%
  merge(MedExp)

panelwAvg <- panelwAvg %>%
  mutate(RBartik_cps = panelwAvgNoOut$RBartik_cps)

dumm <- panelwAvg %>%
  mutate(
    Dpe = pe - Rpe -Tpe + mean(pe, na.rm = TRUE),
    DRegInf = RegInf - RRegInf - TRegInf + mean(RegInf, na.rm = TRUE),
    DBartik_out = Bartik_out - RBartik_out - TBartik_out + mean(Bartik_out, na.rm = TRUE),
    DUNRATE = UNRATE - RUNRATE - TUNRATE + mean(UNRATE, na.rm = TRUE),
    DUNEMPGood = UNEMPGood - RUNEMPGood - TUNEMPGood + mean(UNEMPGood, na.rm = TRUE),
    DFinBetterLast1 = FinBetterLast1 - RFinBetterLast1 - TFinBetterLast1 + mean(FinBetterLast1, na.rm = TRUE),
    DRincUp = RincUp - RRincUp - TRincUp + mean(RincUp, na.rm = TRUE),
    DFinBetterNext1 = FinBetterNext1 - RFinBetterNext1 - TFinBetterNext1 + mean(FinBetterNext1, na.rm = TRUE),
    DBizGood = BizGood - RBizGood - TBizGood + mean(BizGood, na.rm = TRUE),
    DBizExGood = BizExGood - RBizExGood - TBizExGood + mean(BizExGood, na.rm = TRUE),
    DgasExp = gasExp - RgasExp - TgasExp + mean(gasExp, na.rm = TRUE)
    
    
  ) 

Dsub <- dumm %>%
  select(c(survey_date, REGION, yearquarter, Dpe,DRegInf, DBartik_out, DUNRATE, DUNEMPGood, DFinBetterLast1, DRincUp, DFinBetterNext1, DBizGood, DBizExGood, DgasExp))%>%
  rename(pe = Dpe, RegInf = DRegInf, Bartik_out = DBartik_out, UNRATE = DUNRATE,UNEMPGood = DUNEMPGood, FinBetterLast1 = DFinBetterLast1, RincUp = DRincUp, FinBetterNext1 = DFinBetterNext1, BizGood = DBizGood, BizExGood = DBizExGood, gasExp = DgasExp)%>%
  panel(~REGION + survey_date)



est_out <- feols(RegInf ~  l(RegInf,1:2)  | pe  ~ Bartik_out  , Dsub, panel.id = c('REGION', 'survey_date'))


betas = coeftable(est_out)[1:4]

panelStage2 <- panelNoOut %>%
  mutate(st2Inf = RegInf  - betas[2]*pe-betas[3]*lag(RegInf,1)-betas[4]*lag(RegInf,2)  )

panelStage2 <- panelStage2 %>%
  group_by(survey_date) %>%
  mutate(RBartik_cps = mean(Bartik_cps, na.rm = TRUE), Rpe = mean(pe, na.rm = TRUE))

# Jackknife correction;

panel_out2 <- panelwAvg %>%
...
out_bias1 <- feols(RegInf ~ medianExp + FinBetterLast1+ RincUp + BizGood+ BizExGood | REGION  | pe + Rpe  ~ Bartik +RBartik + 1 +RRegInf  , panel_out2, fsplit = ~TimeSample, panel.id = c('REGION', 'survey_date'), vcov = "DK")
out_bias2 <-  feols(RegInf ~ medianExp + FinBetterLast1+ RincUp + BizGood+ BizExGood | REGION  | pe + Rpe ~ Bartik +RBartik + 1 +RRegInf , panel_out2, fsplit = ~RegSample, panel.id = c('REGION', 'survey_date'), vcov = "DK")
c_out1 = coefficients(out_bias1)
c_out2 = coefficients(out_bias2)
s61= se( feols(RegInf ~  medianExp + FinBetterLast1+ RincUp + BizGood+ BizExGood | REGION  | pe + Rpe ~ Bartik +RBartik +1 +RRegInf  , panelwAvg, panel.id = c('REGION', 'survey_date'), vcov = "DK"))
adj_outBeta = 2*c_out1[1]-(c_out2[2]+c_out2[3])/2
adj_outGamma = 2*c_out1[7] - (c_out2[8]+c_out2[9])/2


out_bias3 <- feols(RegInf ~ medianExp | REGION  | pe + Rpe  ~ Bartik +RBartik + 1 +RRegInf  , panel_out2, fsplit = ~TimeSample, panel.id = c('REGION', 'survey_date'), vcov = "DK")
out_bias4 <-  feols(RegInf ~ medianExp  | REGION  | pe + Rpe ~ Bartik +RBartik + 1 +RRegInf , panel_out2, fsplit = ~RegSample, panel.id = c('REGION', 'survey_date'), vcov = "DK")
c_out3 = coefficients(out_bias3)
c_out4 = coefficients(out_bias4)
s62= se( feols(RegInf ~  medianExp | REGION  | pe + Rpe ~ Bartik +RBartik +1 +RRegInf  , panelwAvg, panel.id = c('REGION', 'survey_date'), vcov = "DK"))
adj_outBeta2 = 2*c_out3[1]-(c_out4[2]+c_out4[3])/2
adj_outGamma2 = 2*c_out3[7] - (c_out4[8]+c_out4[9])/2

adjT <- as.tibble(c(  round(adj_outBeta2,4),   round(adj_outGamma2,4))) %>%
  cbind(c("**", "*"))%>%
  cbind(c( round(adj_outBeta,4), round(adj_outGamma,4))) %>%
  cbind(c("**", "*"))

colnames(adjT) <- c("no controls", " ", "controls", " ")
rownames(adjT) <- c("$\\pi^e_R$ coeff.", "$\\pi^e$ coeff.")
kbl(adjT, "latex", booktabs = T, escape = FALSE, caption = "Aggregate pass-through", label = "bias:commeffects:stage2") %>%
  kable_styling(latex_options = "striped") %>%
  footnote(general = "Estimates pass-through of regional inflation expectations and median national expectations using the IV common effects estimator with integrative fixed effects. The first column excludes exogenous controls. The second column includes controls. Applies the Jacknife bias correction.", number = "sign. levels: ** = 0.05, * = 0.10" , threeparttable = T) %>%
  save_kable("tables/biascorrectionsCCE.tex")

# Robust estimates
library(tempdisagg)
# Get State-level CPI
state_cpi <- region_cpi_inflation(statecpi_raw)
paneldataState <- full_join(paneldata,state_cpi)
paneldataState <- paneldataState %>%  panel(~REGION + survey_date, duplicate.method = 'first')
estState <- feols(RegInf ~  l(RegInf,1:2) + UNRATE + UNEMPGood +  FinBetterLast1+ RincUp+ FinBetterNext1+ BizGood+ BizExGood+ gasExp | REGION + yearquarter | pe  ~ Bartik  , paneldataState)

tRobust <- build_robust_table(michigan, michiganFirst, michiganSmall, paneldataState, paneldataFirst, paneldataSmall, paneldataLag)
